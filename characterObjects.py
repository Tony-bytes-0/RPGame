...
#tony por favor , por favor usa estos metodos para mutar objetos porfavor la conchetumadre! -----------
    ...
    def addState(self, state):
        print('estado antes de agregar estados puta madre', self.states)
        if state != NULL:
...
# ------------------ Tecnicas
        ...
        elif select == 'Punto Ciego':
            damage = ((self.attack * 0.5) - (defender.defence * 0.1) + 1 )
            print(self.name, ' realiza un golpe rapido, tomando fuera de guardia a su rival  ' , end='')
            defender.calculateDamage(damage, self)

        elif select == 'Golpe Bajo':
            damage = ((self.attack * 0.3) - (defender.defence * 0.4) + 1 )
            print(self.name, ' Dispara una flecha apuntando a los puntos de apoyo del rival ' , end='')
            defender.calculateDamage(damage, self)
            defender.addSomeStat()
...
class mainCharacter(character):#mc personaje principal
    ...
    def addStat(self, attribute, amount):# --- aqui es donde se agregan las estadisticas ---
        ...
        if attribute == 'Vida':
            print(self.life, attribute, amount)
            self.life = self.life +  amount
    ...
    def export(self):#no he terminado esto, trabajando aqui
        allSkills = " ".join(self.skills)

        print(allSkills)

        allSkills = " ".join(self.tecniques)
        print(allSkills)
...
class enemy(character):
    ...
    def turn(self, player):
        ...
        print('esta es la tirada de dados del turno del enemigo:',op)
        time.sleep(0.3)
...
#-----------------------------------------------------------
            ...
            if self.energy >= 1:#mientras halla 1 de energia
                if op <= 7 and self.skillsAndTecniquesAvalible(False) != False:
